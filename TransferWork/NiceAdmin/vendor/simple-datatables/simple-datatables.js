            ...
            class n {
                ...
                build(t) {
                    ...
                    return i.length || (i = t.map((() => ""))), i.forEach(((i, a) => {
                        const n = e("td");
                        t[a] && t[a].length || (t[a] = ""), n.innerHTML = t[a], n.data = t[a], n.classList = SubString(t[a], 'class="', '"'), s.appendChild(n)
                ...
                add(t) {
                    if (Array.isArray(t)) {
                        ...
                        Array.isArray(t[0]) ? t.forEach((t => {
                            e.data.unshift(this.build(t))
                        })) : e.data.unshift(this.build(t)), e.data.length && (e.hasRows = !0), this.update(), e.columns().rebuild()
                    }
                }
                remove(t) {
                    const e = this.dt;
            ...
            class o {
                ...
                render(t) {
                    ...
                    if ($('#titlePage').text() == 'EVM') {
                        btnhtml = "<button class='btn btn-outline-primary' onclick='AddEVM_Open()'>Add New EVM</button>";                        
                    }
                    else if ($('#titlePage').text() == 'Dashboard') {
                        btnhtml = '';
                    }
                    else {
                        btnhtml = "<button class='btn btn-outline-primary' onclick='AddModal_Open()'>Add New Work</button>";
                        btnhtml += "<button class='btn ms-3 btn-outline-success' onclick='ExcelExport()'>Export</button>";
                    }                 
                    ...
                    }), i += "<div class='dataTable-top'>" + btnhtml,
                        i += s.layout.top, i += "</div>",
                    ...
                    if (s.searchable) {
                        const t = `<div class="header">
								   <div class="search-bar">
								   <div class="search-form d-flex align-items-center">
										<input class="dataTable-input" type="text" name="query" placeholder="${s.labels.placeholder}" title="Enter search keyword">
                ...
                search(t, c) {
                    const previousSearchData = new Set(this.searchData);

                    if (c != null) {
                        const column = this.columns(c);
                        const searchData = new Set();
                        if (previousSearchData.size > 0) {
                            // Tìm kiếm trên kết quả tìm kiếm trước đó
                            previousSearchData.forEach((index) => {
                                const row = this.data[index];
                                if (column.visible()) {
                                    let cellContent = row.cells[c].textContent;
                                    if (row.cells[c].hasAttribute("data-content")) {
                                        cellContent = row.cells[c].getAttribute("data-content");
                                    }
                                    cellContent = cellContent.toLowerCase();
                                    if (cellContent.includes(t.toLowerCase())) {
                                        searchData.add(index);
                                    }
                                }
                            });
                        } else {
                            // Tìm kiếm như bình thường
                        ...
                        if (searchData.size > 0) {
                            this.searchData = Array.from(searchData);
                            this.currentPage = 1;
                            this.searching = true;
                            this.clear();
...
var searchData;
function SubString(str, startStr, endStr) {
    try {
        if (!str.includes(startStr)) {
            return str;
        }
        let StartPos = str.indexOf(startStr) + startStr.length;
        let dataBack = str.substring(StartPos, str.length);
        dataBack = dataBack.substring(0, dataBack.indexOf(endStr));
